...
class StoreModel extends Model {
    public function hasPurchased($id, $type = 'track', $force = false) {
        ...
        if ($type == 'track') {
            ...
            $query = $this->db->query("SELECT id FROM transactions WHERE type_id=? AND userid=? AND type=?", $id, model('user')->authId, 'album');
            return $query->rowCount();
        }
    }
    ...
    public function getTotalItemSold() {
        $tracks = implode(',', $this->getAllMyTracks());
        $albums = implode(',', $this->getAllMAlbums());
        $query = $this->db->query("SELECT id FROM transactions WHERE (type=? AND type_id IN ($tracks)) OR (type=? AND type_id IN ($albums))", 'track', 'album');
        return $query->rowCount();
    }
