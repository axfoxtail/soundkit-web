...
class TrackModel extends Model {
    ...
    public function getExpiredArtists($typeId = null) {
        ...
        if (config('disable-tracks', true) and config('enable-premium', false)) {
            $query = $this->db->query("SELECT DISTINCT(userid) FROM transactions as tab1 WHERE type = ? AND valid_time < $time AND (SELECT COUNT(id) as c FROM transactions as tab2 WHERE valid_time > $time AND tab2.userid = tab1.userid AND type='pro') < 1 ORDER BY userid DESC ", 'pro');
