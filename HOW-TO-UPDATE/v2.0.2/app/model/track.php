...
class TrackModel extends Model {
    ...
    public function replaceWaveImages($limit = 50, $offset = 0, $type = 0) {
        $query = $this->db->query("SELECT track_file,demo_file,id,wave,wave_colored,demo_wave,demo_wave_colored FROM tracks ORDER BY id ASC LIMIT $limit OFFSET $offset");
        $result = false;
        while($fetch = $query->fetch(PDO::FETCH_ASSOC)) {
            try {
                $wave = $fetch['wave'];
