...
Hook::getInstance()->register('track.upload.data', function($val, $i) {
    $C = getController();
    ...
    if ($track['price'] > 0) {
        ...
        if (!model('store::store')->hasPurchased($track['id'])) {
            ...
            if ($track['demo_file']) {
                ...
                $file = getController()->getFileUri($track['demo_file'], false);
...
Hook::getInstance()->register('track.query', function($result, $type, $typeId) {
    ...
    switch($type) {
        case 'store-browse':
            ...
            if ($gType == 'tracks') {
                $sql = "SELECT * FROM tracks WHERE status=? AND public = ?  AND approved=? AND price > 0";
                $param = array(1, 1, 1);
                ...
                $sql = "SELECT * FROM playlist WHERE  playlist_type=? AND public = ? AND price > 0";
                $sql .= " ORDER BY id DESC ";
                $playlistType = 0;
                $param[] = $playlistType;
                $param[] = 1;
        ...
        case 'store-best-week':
            ...
            $sql = "SELECT *,(SELECT count(track) FROM views WHERE views.track=tracks.id AND views.time BETWEEN $time AND $currentTime ) as count FROM tracks WHERE tracks.status=? AND  userid NOT IN ($blockedIds) AND price > 0  AND (tracks.public != ?)  AND approved=?";
            $param = array(1, 3,1);
        ...
        case 'store-top-albums':
            $sql = "SELECT *,(SELECT count(typeid) as types FROM likes WHERE type=? AND typeid=playlist.id) as count FROM playlist WHERE  playlist_type=? AND public = ? AND price > 0";
            $sql .= " ORDER BY count DESC ";
