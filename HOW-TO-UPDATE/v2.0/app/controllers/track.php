...
class TrackController extends Controller {
    ...
    public function upload() {
        ...
        if ($uploader->passed()) {
            ...
            if (config('wave-generator', 'browser') == 'server') {
                ...
                $scale = 'lin';
                shell_exec("".config('ffmpeg-path')." -y -i $file -filter_complex \"aformat=channel_layouts=mono,compand=gain=-3,showwavespic=s=600x120:scale=$scale:colors=".$color.",drawbox=x=(iw-w)/2:y=(ih-h)/2:w=iw:h=1:color=white\" -frames:v 1 $outputWave 2>&1");
                shell_exec("".config('ffmpeg-path')." -y -i $file -filter_complex \"aformat=channel_layouts=mono,compand=gain=-3,showwavespic=s=600x120:scale=$scale:colors=".$colored.",drawbox=x=(iw-w)/2:y=(ih-h)/2:w=iw:h=1:color=white\" -frames:v 1 $outputWaveColored 2>&1");
    ...
    public  function _getTrackDetail($trackId) {
        ...
        $file = $this->getFileUri($track['track_file']);
