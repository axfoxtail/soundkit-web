...
class MP3File
{
    ...
    public function getDuration($use_cbr_estimate=false)
    {
        ...
        while (!feof($fd))
        {
            ...
            else if ($block[0]=="\xff" && (ord($block[1])&0xe0) )
            {
                ...
                $duration += (!$info['Samples']) ? 0 : ( $info['Samples'] / $info['Sampling Rate'] );
            }
    ...
    public static function parseFrameHeader($fourbytes)
    {
        ...
        $info['Samples'] = (isset($samples[$simple_version][$layer])) ? $samples[$simple_version][$layer] : 0;
        return $info;
    }
    ...
    private static function framesize($layer, $bitrate,$sample_rate,$padding_bit)
    {
        ...
        if ($layer==1) {
            $value = ($sample_rate) ? (12 * $bitrate*1000 /$sample_rate) : 0;
            return intval(($value + $padding_bit) * 4);
                }
        else {
            $value = ($sample_rate) ? (144 * $bitrate*1000 /$sample_rate) : 0;
            return intval($value + $padding_bit);
        }
