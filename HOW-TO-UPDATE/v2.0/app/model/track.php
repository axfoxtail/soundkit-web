...
class TrackModel extends Model {
    ...
    public function getExpiredArtists($typeId = null) {
        ...
        if (config('disable-tracks', true) and config('enable-premium', false)) {
            $query = $this->db->query("SELECT DISTINCT(userid) FROM transactions as tab1 WHERE type = ? AND valid_time < $time AND (SELECT COUNT(id) as c FROM transactions as tab2 WHERE valid_time > $time AND tab2.userid = tab1.userid AND type='pro') < 1 ORDER BY id DESC ", 'pro');
    ...
    public function getTracks($type,$typeId,$offset,$limit = 10) {
        ...
        if ($type == 'feed') {
            ...
            $sql = "SELECT *,(SELECT count(track) FROM views WHERE views.track=tracks.id ) as count FROM tracks WHERE tracks.status=? AND  userid NOT IN ($blockedIds) AND userid NOT IN ($expiredArtists) AND  tracks.time BETWEEN $time AND $currentTime  AND (public != ?)  AND approved=?";
            $param = array(1, 3,1);
            if ($genre != 'all') {
                $param[] = $genre;
                $sql .= ' AND genre=? ';
            ...
            $sql = "SELECT *,(SELECT count(track) FROM views WHERE views.track=tracks.id AND views.time BETWEEN $time AND $currentTime ) as count FROM tracks WHERE tracks.status=? AND userid NOT IN ($expiredArtists)  AND  userid NOT IN ($blockedIds)   AND (tracks.public != ?)  AND approved=?";
            $param = array(1, 3,1);
            if ($genre != 'all') {
                $param[] = $genre;
                $sql .= ' AND genre=? ';
            ...
            if ($entries) {
                ...
                $sql = "SELECT * FROM  tracks  WHERE status=? AND id IN ($entries)  AND approved=? ORDER BY id DESC";
                $param = array(1,1);
            } else {
                return array();
            }
    ...
    public function listPlaylists($type, $typeId, $limit = 20, $offset = 0) {
        ...
        if ($typeId == 'collection') {
            ...
            $sql = "SELECT * FROM playlist WHERE (userid=?) AND playlist_type=? AND (public != ? OR userid=?) ";
            $sql .= " ORDER BY time DESC LIMIT $limit OFFSET $offset";
            $playlistType = ($type == 'playlist') ? 1 : 0;
            $query = $this->db->query($sql, $profileId, $playlistType, 3, $userid);
            return $query->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function getPlaylistFirstTrack($id) {
        $query = $this->db->query("SELECT track FROM playlistentries WHERE playlist=?  ORDER BY id DESC", $id);
        if ($query) {
    ...
    public function getPlaylistTracks($id,$trackId) {
        $query = $this->db->query("SELECT track FROM playlistentries WHERE playlist=? AND track !=?  ORDER BY id DESC", $id, $trackId);
        $result = array();
        if ($query) {
            while($fetch = $query->fetch(PDO::FETCH_ASSOC)) {
                $track = $this->findTrack($fetch['track']);
    ...
    public function getViews($id ,$limit = 10, $offset = 0) {
        ...
        $sql .= " ORDER BY viewid DESC LIMIT {$limit} OFFSET {$offset} ";
    ...
    public function replaceWaveImages($limit = 50, $offset = 0, $type = 0) {
        ...
        while($fetch = $query->fetch(PDO::FETCH_ASSOC)) {
            try {
                ...
                if ($fetch['wave'] and file_exists(path($fetch['wave']))) {
                    $dir = md5($fetch['wave']);
                    if (!is_dir(path('uploads/waves/'.$dir))) {
                        @mkdir(path('uploads/waves/'.$dir), 0777, true);
                        $file = @fopen(path("uploads/waves/$dir/index.html"), 'x+');
                    ...
                    if (!$type) {
                        ...
                        $output = path($wave);
                        shell_exec("".config('ffmpeg-path')." -y -i $trackFile -filter_complex \"aformat=channel_layouts=mono,compand,showwavespic=s=600x120:colors=".$color."\" -frames:v 1 $output 2>&1");
                    }
                    delete_file(path($fetch['wave']));
                }
                if ($fetch['wave_colored']  and file_exists(path($fetch['wave_colored'])))  {
                    ...
                    if (!$type) {
                        ...
                        shell_exec("".config('ffmpeg-path')." -y -i $trackFile -filter_complex \"aformat=channel_layouts=mono,compand,showwavespic=s=600x120:colors=".$colored."\" -frames:v 1 $output 2>&1");
                    }
                    delete_file(path($fetch['wave_colored']));
                }

                if ($fetch['demo_wave']  and file_exists(path($fetch['demo_wave']))) {
                    ...
                    if (!$type) {
                        ...
                        shell_exec("".config('ffmpeg-path')." -y -i $demoFile -filter_complex \"aformat=channel_layouts=mono,compand,showwavespic=s=600x120:colors=".$color."\" -frames:v 1 $output 2>&1");
                    }
                    delete_file(path($fetch['demo_wave']));
                }

                if ($fetch['demo_wave_colored']   and file_exists(path($fetch['demo_wave_colored']))) {
                    ...
                    if (!$type) {
                        ...
                        shell_exec("".config('ffmpeg-path')." -y -i $demoFile -filter_complex \"aformat=channel_layouts=mono,compand,showwavespic=s=600x120:colors=".$colored."\" -frames:v 1 $output 2>&1");
                    }
